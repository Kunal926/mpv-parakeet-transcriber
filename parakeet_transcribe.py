...
# --- Helper utilities for modular transcription pipeline ---


class ParakeetTranscriptionError(RuntimeError):
    """Raised for recoverable errors during the transcription pipeline."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "audio_file_path",
        type=str,
        help="Path to the input audio file (e.g., WAV). Expected to be 16kHz mono.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Global start time offset in seconds from the original media to apply to all timestamps.",
    )
    parser.add_argument(
        "--force_float32",
        action="store_true",
        help="Force the model to run in float32 precision on GPU, even if bfloat16/float16 is available.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.0,
        help="Maximum subtitle duration in seconds for word segmentation",
    )
    parser.add_argument(
        "--pause",
        type=float,
        default=0.6,
        help="Inter-word pause (s) that triggers a new subtitle when using word segmentation",
    )
    ...
    parser.add_argument("--coalesce_gap_ms", type=int, default=360, help="Merge consecutive events if gap ≤ this and 2-line fit/cps ok")
    parser.add_argument(
        "--two_line_threshold",
        type=float,
        default=0.60,
    ...
    parser.add_argument("--max_merge_gap_ms", type=int, default=360, help="Max gap for orphan merges/borrowing")
    parser.add_argument(
        "--run_dir",
        type=str,
        default=None,
...
def write_error_srt(srt_path: str, audio_path: str, message: str) -> None:
...
def load_model(force_float32: bool):
    """Load the ASR model and place it on the appropriate device."""
    ...
    try:
        ...
        raise ParakeetTranscriptionError("Model download failed") from model_load_exc
    ...
    if use_cuda:
        ...
        if force_float32:
            ...
            try:
                ...
                print(
                    f"Warning: Could not convert model to bfloat16/float16: {precision_exc}. Using float32 on GPU.",
                    file=sys.stderr,
                )
                asr_model = asr_model.to(dtype=torch.float32)
                model_dtype = torch.float32
    ...
    print(
        f"Model expects sample rate: {target_sr_from_model_cfg} Hz. Input audio should match this.",
        file=sys.stderr,
    )
    return asr_model, model_dtype, use_cuda, load_time, target_sr_from_model_cfg


def get_audio_info(audio_path: str, target_sr_from_model_cfg: int) -> Tuple[float, Optional[int], Optional[int]]:
    """Inspect the input audio file and return (duration, samplerate, channels)."""

    print(
        f"Getting audio info for: '{audio_path}' (expected to be pre-processed)...",
        file=sys.stderr,
    )
    ...
    try:
        ...
        if actual_sr != target_sr_from_model_cfg:
            print(
                f"CRITICAL WARNING: Input audio sample rate ({actual_sr}Hz) differs from model's expected rate ({target_sr_from_model_cfg}Hz). Results may be suboptimal.",
                file=sys.stderr,
            )
        if actual_channels != 1:
            print(
                f"CRITICAL WARNING: Input audio has {actual_channels} channels. Model expects mono (1 channel). Results may be suboptimal.",
                file=sys.stderr,
            )
    except Exception as info_exc:
        print(
            f"Warning: Could not get audio info using soundfile: {info_exc}. Attempting with librosa for duration.",
            file=sys.stderr,
        )
        try:
            audio_duration_seconds = librosa.get_duration(path=audio_path)
            print(
                f"Input audio duration (via librosa): {audio_duration_seconds:.2f}s",
                file=sys.stderr,
            )
        except Exception as librosa_exc:
            raise ParakeetTranscriptionError(f"Could not determine audio duration: {librosa_exc}") from librosa_exc
    ...
    return audio_duration_seconds, actual_sr, actual_channels


def apply_long_audio_settings_if_needed(asr_model, audio_duration_seconds: float, threshold_s: float = 480.0) -> bool:
    """Adjust the model configuration when the audio is longer than the threshold."""

    if audio_duration_seconds <= threshold_s:
        return False

    _log_event("long_audio_apply_attempt", threshold_s=threshold_s, dur_s=audio_duration_seconds)
    try:
        print(
            f"Audio duration ({audio_duration_seconds:.2f}s) > {threshold_s}s. Applying long audio settings.",
            file=sys.stderr,
        )
        ...
        print(
            f"Warning: Failed to apply long audio settings: {setting_exc}. Proceeding without them.",
            file=sys.stderr,
        )
        _log_event("long_audio_applied", ok=False, error=str(setting_exc))
        return False
...
def transcribe_audio(
    ...
    """Run the ASR model and build segment dictionaries ready for post-processing."""
    ...
    with autocast(dtype=model_dtype if use_amp_autocast else torch.float32, enabled=use_amp_autocast):
        print(
            f"Transcribing with precision: {model_dtype if use_cuda else 'float32 (CPU)'}. Autocast enabled: {use_amp_autocast}",
            file=sys.stderr,
        )
    ...
    _log_event("transcribe_done", asr_s=round(elapsed, 3))

    if not output_from_transcribe or not isinstance(output_from_transcribe, list) or not output_from_transcribe[0]:
        raise ParakeetTranscriptionError("Transcription failed or produced no hypotheses")
    ...
    if hasattr(first_result, "text") and hasattr(first_result, "timestamp") and first_result.timestamp is not None:
        ...
        raise ParakeetTranscriptionError("Transcription output format not recognized or timestamp data missing")
    ...
    if not segments:
        raise ParakeetTranscriptionError("No transcript text available")

    return segments, elapsed


def apply_global_offset(segments: List[dict], global_offset_seconds: float) -> None:
...
def cleanup_model(asr_model, long_audio_settings_applied: bool) -> None:
    ...
    if asr_model is not None:
        if long_audio_settings_applied:
            ...
            try:
                ...
                print(f"Warning: Failed to revert long audio settings: {revert_exc}", file=sys.stderr)
            finally:
        ...
        try:
            ...
            print(f"Error during model cleanup: {cleanup_exc}", file=sys.stderr)
        finally:
        ...
        try:
            ...
            print(f"Error during cache cleanup: {cache_exc}", file=sys.stderr)
        finally:
...
def main():
    ...
    if not os.path.exists(audio_path):
        ...
        write_error_srt(srt_path, audio_path, f"Audio file not found: {os.path.basename(audio_path)}")
        sys.exit(1)
    ...
    try:
        ...
        asr_model, model_dtype, use_cuda, load_time, target_sr_from_model_cfg = load_model(force_float32)

        audio_duration_seconds, _, _ = get_audio_info(audio_path, target_sr_from_model_cfg)

        long_audio_settings_applied = apply_long_audio_settings_if_needed(asr_model, audio_duration_seconds)

        segments, asr_time = transcribe_audio(
            asr_model,
            audio_path,
            model_dtype,
            use_cuda,
        ...
        apply_global_offset(segments, global_offset_seconds)
        ...
        _log_event("postprocess_done", out_events=len(processed), postproc_s=round(post_time, 3))
        ...
        _log_event(
            ...
            asr_s=round(asr_time, 3),
            postproc_s=round(post_time, 3),
        ...
        write_error_srt(srt_path, audio_path, "CUDA OutOfMemoryError")
        sys.exit(1)
        ...
        write_error_srt(srt_path, audio_path, "Transcription interrupted")
        sys.exit(1)
    except ParakeetTranscriptionError as pipeline_exc:
        err_msg = f"Error: {pipeline_exc}"
        print(err_msg, file=sys.stderr)
        write_error_srt(srt_path, audio_path, str(pipeline_exc))
        sys.exit(1)
        ...
        write_error_srt(srt_path, audio_path, f"Error during transcription: {str(e)[:100]}")
        sys.exit(1)
    finally:
        cleanup_model(asr_model, long_audio_settings_applied)
